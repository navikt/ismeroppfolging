...
class KartleggingssporsmalServiceTest {
    ...
    fun `processOppfolgingstilfelle should store in db when oppfolgingstilfelle is relevant for planlagt kandidat`() {
        ...
        assertEquals(
            oppfolgingstilfelleInsideStoppunktInterval.tilfelleStart.plusDays(KARTLEGGINGSSPORSMAL_STOPPUNKT_START_DAYS),
    ...
    fun `processOppfolgingstilfelle should store in db when oppfolgingstilfelle is exactly at stoppunkt start`() {
        ...
        assertEquals(
            oppfolgingstilfelleAtStoppunktStart.tilfelleStart.plusDays(KARTLEGGINGSSPORSMAL_STOPPUNKT_START_DAYS),
            stoppunkter.first().stoppunktAt,
        )
    }
    ...
    fun `processOppfolgingstilfelle should store in db when oppfolgingstilfelle is exactly at stoppunkt end`() {
        ...
        assertEquals(
            oppfolgingstilfelleAtStoppunktEnd.tilfelleStart.plusDays(KARTLEGGINGSSPORSMAL_STOPPUNKT_START_DAYS),
            stoppunkter.first().stoppunktAt,
        )
    }
    ...
    fun `processOppfolgingstilfelle should store in db when tilfelle ending exactly 30 days ago`() {
        ...
        assertEquals(
            oppfolgingstilfelleExactly30DaysAgo.tilfelleStart.plusDays(KARTLEGGINGSSPORSMAL_STOPPUNKT_START_DAYS),
            stoppunkter.first().stoppunktAt,
        )
    }
    ...
    fun `processOppfolgingstilfelle should store in db when antallSykedager is null but tilfelle interval within stoppunkt interval`() {
        ...
        assertEquals(
            oppfolgingstilfelleWithNullSykedager.tilfelleStart.plusDays(KARTLEGGINGSSPORSMAL_STOPPUNKT_START_DAYS),
            stoppunkter.first().stoppunktAt,
        )
    }
